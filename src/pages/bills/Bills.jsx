    ...
    <div>
      <div className="bill-main-container">
        ...
        <div className="bill-right">
              ...
              {!loding &&<div className={`bill-middle ${open!=='' && 'blur'}`} >
                <div className={`bill-wrapper  ${data?.items?.length>=3 ? 'grid' : 'flexbox' }`}>
                <div className="add-card" onClick={()=>setOpen('add')}>+</div>
                {data?.items?.map((item)=>(<Billcard key={item?.id} setOpen={setOpen} setItem={setItem} item={item} projectname={projectname} Id={projectId}/>))}
                </div>
              </div>}
          {(open==='update' || open==='add') && <Billpopup setOpen={setOpen} item={item} setItem={setItem} setChange={setChange} change={change} open={open}/>}
        </div>
      </div>
    </div>
  )
